...
while i <= 9:
   ...
   plt.title("number:"+str(i-1)+" Predicted Label: " + str(prediction[0]))
   plt.axis('off')
   plt.show()
   
i = 0
while i <= 9:
   image = Image.open("digits_"+str(i-1-1) + '.png').convert('L')  # 手書き画像のパスを指定して読み込む
   image = image.resize((28, 28))  # 28x28にリサイズ
   image = np.array(image)  # NumPy配列に変換
   image = image.reshape(1, -1)  # 1次元の特徴量ベクトルに変換
   image = 255.0 - image  # 色の反転
   prediction = model.predict(image)  # モデルによる予測
   print(prediction)
   print(type(prediction))
   i += 1
